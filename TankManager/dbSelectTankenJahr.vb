Module dbSelectTankenJahr

    'Abfrage: Tanken Jahr
    Public Function MySqlSelectTankenJahr(selectedJahr)
...
ROUND(`tankmanager`.`tbl_tanken`.`liter` * 100 / `tankmanager`.`tbl_tanken`.`gefahren`, 2) AS verbrauch
FROM `tankmanager`.`tbl_tanken` WHERE DATE_FORMAT(`tankmanager`.`tbl_tanken`.`datum`, '%Y') = '" & selectedJahr & "'
